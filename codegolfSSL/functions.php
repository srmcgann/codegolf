<?
	...
	function drawNewAppletButton(){
		...
		<div id="landingDiv">
			<div class="newAppletButton" id="newAppletButton">
				<img src="/favicon.png" class="NewAppletButtonGraphic">
				Create a New Applet
			</div>
		</div>
		<?		
	...
	function drawLoggedOutMenu(){
		...
		<div id="rightMenu">
			<span onclick="Login()" class="rightMenuLink">Login</span>
			<span class="menuText">or</span>
			<span onclick="Register()" class="rightMenuLink">Register</span>
		</div>
		<div id="header"></div>
	...
	function drawMenu(){
		...
		?>
		<a href="/" class="featuredLink"><img class="logoImage" src="/codegolf.png" /></a>
	...
	function drawApplet($row,$drawLegend=0){
		...
		<div class="appletDiv" id="appletDiv<?=$id?>">
			...
			<div class="code-input"><div class="function-wrap" style="float:left;">function u(t){</div><input id="webglCheckbox<?=$id?>" <?if($webgl)echo "checked"?> style="margin-left:5%;float:left;" type="checkbox"></input><span style="float:left;margin-left:5px;font-size:.7em;" id="webglCheckboxLabel<?=$id?>">webgl</span>
				...
				if(isset($_COOKIE['id']) && isset($_COOKIE['session'])){
					?>
					<button id="postRemix<?=$id?>" onclick="saveApplet(<?=$id?>,<?=$userID?>,<?=$id?>)" style="float:right;display:none;">Post Remix</button>
					<?
				}else{
					?>
					<span id="postRemix<?=$id?>" style="display:none;font-size:.8em;margin-left:20px;">Please login or register to post your work.</span>
					<?
				}
				?>
				<script>
					...
					var editor<?=$id?> = CodeMirror.fromTextArea($("#textArea<?=$id?>")[0],{theme:"midnight",lineWrapping:true});
			...
			<table class="userInfoTable">
				<tr>
					<td style="border:0"><a href="/<?=$name?>"><img src="<?=$avatar?>" class="appletAvatar" />
